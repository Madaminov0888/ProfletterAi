...
/**
 * Builds (and regenerates) the letter plan / outline.
 *
 * The model is asked to return JSON; we keep the Kotlin DTO local to this
 * service so the rest of the app keeps using the clean [LetterPlan] model.
 */
class PlanService(
    ...
    suspend fun generatePlan(
        ...
        val dto = runCatching { json.decodeFromString(PlanDto.serializer(), cleaned) }
            .getOrElse {
                // Fallback: produce a "plan" out of whatever the model returned
                PlanDto(
